...
def update_flash_sale(
    ...
    fs = db.query(database.FlashSale).filter(
        database.FlashSale.id == sale_id,
        database.FlashSale.deleted_at == None
    ).first()
    if not fs:
        raise HTTPException(status_code=404, detail="Flash sale not found")

    if data.title is not None: fs.title = data.title
    if data.description is not None: fs.description = data.description
    if data.discount_type is not None: fs.discount_type = data.discount_type
    if data.discount_value is not None: fs.discount_value = data.discount_value
    if data.starts_at is not None: fs.starts_at = data.starts_at
    if data.ends_at is not None: fs.ends_at = data.ends_at
    if data.is_active is not None: fs.is_active = data.is_active
    ...
    if data.product_ids is not None:
        ...
        for pid in data.product_ids:
            db.add(database.FlashSaleItem(flash_sale_id=sale_id, product_service_id=pid))
