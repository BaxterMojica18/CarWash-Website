...
def create_invoice(
    ...
    current_user=Depends(has_permission("manage_invoices")),
):
    ...
    log_audit(
        ...
        "invoice",
        result.id,
        {"invoice_number": result.invoice_number, "customer": invoice.customer_name},
...
def download_invoice_pdf(
    ...
    # Title
    title = Paragraph(
        f"<b>Invoice Receipt #{invoice.invoice_number}</b>", styles["Title"]
    )
    ...
    # Invoice details
    details = Paragraph(
        f"<b>Date:</b> {invoice.date.strftime('%Y-%m-%d %H:%M')}<br/>"
        f"<b>Customer:</b> {invoice.customer_name}<br/>"
        f"<b>Location:</b> {invoice.location.name}",
        styles["Normal"],
    )
    ...
    for item in invoice.items:
        product = (
            ...
            .filter(database.ProductService.id == item.product_service_id)
            .first()
        )
...
def download_invoice_jpg(
    invoice_id: int,
    db: Session = Depends(database.get_db),
    current_user=Depends(get_current_user_optional_query),
):
    ...
    y = 50
    draw.text(
        (250, y), f"Invoice #{invoice.invoice_number}", fill="black", font=font_title
    )

    y += 70
    draw.text(
        (50, y),
        f"Date: {invoice.date.strftime('%Y-%m-%d %H:%M')}",
        fill="black",
        font=font_normal,
    )
    y += 35
    draw.text(
        (50, y), f"Customer: {invoice.customer_name}", fill="black", font=font_normal
    )
    y += 35
    draw.text(
        (50, y), f"Location: {invoice.location.name}", fill="black", font=font_normal
    )
    ...
    for item in invoice.items:
        ...
        draw.text(
            (480, y + 5), f"${item.unit_price:.2f}", fill="black", font=font_normal
        )
    ...
    y += 40
    draw.text(
        (480, y), f"TOTAL: ${invoice.total_amount:.2f}", fill="black", font=font_bold
    )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="invoice-{invoice.invoice_number}.jpg"'
        },
    )
