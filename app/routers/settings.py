...
def create_location(
    ...
    current_user=Depends(has_permission("manage_locations")),
):
    result = crud.create_location(db, location, current_user.id)
    log_audit(
        db,
        current_user.id,
        "create",
...
def create_product(
    product: schemas.ProductServiceCreate,
    request: Request,
    db: Session = Depends(database.get_db),
...
def save_theme(
    theme: schemas.CustomThemeCreate,
    db: Session = Depends(database.get_db),
...
def update_theme(
    theme_id: int,
    theme_data: schemas.CustomThemeCreate,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user=Depends(has_permission("manage_settings")),
):
    ...
    return {"message": "Theme updated", "id": theme.id}
...
def save_business(
    info: schemas.BusinessInfoCreate,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user=Depends(has_permission("manage_settings")),
):
    owner_id = get_business_owner_id(db, current_user)
    result = crud.save_business_info(db, owner_id, info)
    log_audit(
        db,
        current_user.id,
        "update",
...
def save_invoice_custom(
    custom: schemas.InvoiceCustomizationCreate,
    db: Session = Depends(database.get_db),
    current_user=Depends(has_permission("manage_settings")),
):
    owner_id = get_business_owner_id(db, current_user)
...
@router.post("/phone")
def update_phone(
    phone_data: schemas.UserPhoneUpdate,
    db: Session = Depends(database.get_db),
    current_user=Depends(get_current_user),
):
...
def create_payment_method(
    ...
    result = crud.create_payment_method(db, pm.name, pm.icon, pm.is_active)
    log_audit(
        db,
        current_user.id,
        "create",
...
def save_notification_email(
    ...
    info = (
        ...
        .filter(database.BusinessInfo.user_id == owner_id)
        .first()
    )
    if info:
        info.email = email
...
def join_business(
    ...
    owner = (
        db.query(database.User).filter(database.User.business_number == code).first()
    )
    if not owner:
        raise HTTPException(status_code=404, detail="Invalid business code")
    if current_user.business_number == code:
        raise HTTPException(
            status_code=400, detail="You are already part of this business"
        )
