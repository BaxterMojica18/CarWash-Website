...
def create_coupon(
    ...
    existing = db.query(database.Coupon).filter(
...
def update_coupon(
    ...
    coupon = db.query(database.Coupon).filter(
        database.Coupon.id == coupon_id,
        database.Coupon.deleted_at == None
    ).first()
    if not coupon:
        raise HTTPException(status_code=404, detail="Coupon not found")

    if data.description is not None: coupon.description = data.description
    if data.discount_type is not None: coupon.discount_type = data.discount_type
    if data.discount_value is not None: coupon.discount_value = data.discount_value
    if data.min_spend is not None: coupon.min_spend = data.min_spend
    if data.max_uses is not None: coupon.max_uses = data.max_uses
    if data.stock is not None: coupon.stock = data.stock
    if data.expires_at is not None: coupon.expires_at = data.expires_at
