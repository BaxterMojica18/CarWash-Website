...
def add_to_cart(item: schemas.CartItemCreate, db: Session = Depends(database.get_db), current_user = Depends(is_client)):
...
def update_cart_item(item_id: int, item: schemas.CartItemUpdate, db: Session = Depends(database.get_db), current_user = Depends(is_client)):
...
def remove_cart_item(item_id: int, db: Session = Depends(database.get_db), current_user = Depends(is_client)):
...
def clear_cart(db: Session = Depends(database.get_db), current_user = Depends(is_client)):
