...
def get_current_user_optional_query(
    token: Optional[str] = Query(None), db: Session = Depends(database.get_db)
):
    if not token:
...
def login(credentials: schemas.UserLogin, db: Session = Depends(database.get_db)):
    ...
    access_token = create_access_token(
        data={"sub": user.email, "is_demo": user.is_demo}
    )
    return {
...
def firebase_login(
    request: schemas.FirebaseLoginRequest, db: Session = Depends(database.get_db)
):
    ...
    user = crud.get_or_create_firebase_user(
        db, email=request.email, name=request.display_name
    )
...
def get_my_permissions(
    ...
    if True:  # unconditional fetch for the user
        user_hidden_settings = (
            db.query(database.UserSidebarSetting)
            .filter(
...
def list_users(
    ...
    if current_user.business_number:
        users = (
            ...
            .filter(database.User.business_number == current_user.business_number)
            .all()
        )
    else:
    ...
    for user in users:
        user_roles = [
            role.name for role in user.roles if not role.name.startswith("custom_")
        ]
...
def update_user_permissions(
    data: schemas.UpdateUserPermissions,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: database.User = Depends(is_admin_or_owner),
):
    ...
    custom_role = (
        db.query(database.Role).filter(database.Role.name == custom_role_name).first()
    )

    if not custom_role:
        custom_role = database.Role(
            name=custom_role_name, description=f"Custom permissions for user {user.id}"
        )
        db.add(custom_role)
        db.commit()
    ...
    for perm_name in data.permissions:
        perm = (
            db.query(database.Permission)
            .filter(database.Permission.name == perm_name)
            .first()
        )
        if perm:
...
def delete_user(
    ...
    if (
        ...
        and user.business_number != current_user.business_number
    ):
        raise HTTPException(
...
def get_all_roles(
    ...
    roles = (
        db.query(database.Role).filter(~database.Role.name.startswith("custom_")).all()
    )
    result = []
    for role in roles:
...
def update_user_sidebar_settings(
    user_id: int,
    data: schemas.UpdateSidebarSettings,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: database.User = Depends(is_admin_or_owner),
):
    ...
    for page_name, is_visible in data.settings.items():
        ...
        if setting:
            ...
            setting = database.UserSidebarSetting(
                user_id=user_id,
                page_name=page_name,
...
def forgot_password(
    ...
    if not user:
        try:
            ...
            user = crud.get_or_create_firebase_user(
                db, email=request.email, name=fb_user.display_name or "User"
            )
            print(
                f"[AUTH] Recovered Firebase user {request.email} into local DB for password reset."
            )
        except Exception as e:
...
def verify_otp(
    request: schemas.VerifyOtpRequest, db: Session = Depends(database.get_db)
):
...
def reset_password(
    request: schemas.ResetPasswordRequest, db: Session = Depends(database.get_db)
):
    ...
    try:
        ...
        try:
            ...
            print(
                f"[AUTH] Successfully synced new password to Firebase for {user.email}"
            )
        except firebase_auth.UserNotFoundError:
            print(
                f"[AUTH] Firebase user not found for {user.email}, skipping Firebase password update."
            )
    except Exception as e:
