...
def get_business_user_ids(db: Session, user: database.User) -> TypingList[int]:
    ...
    users = (
        ...
        .filter(database.User.business_number == user.business_number)
        .all()
    )
...
def register_user(db: Session, reg_data: schemas.UserRegister):
    ...
    if reg_data.account_type == "owner":
        ...
        if not owner:
            ...
            raise Exception(
                "Invalid business number. Please ask your business owner for the correct code."
            )
    ...
    profile = database.UserProfile(
        user_id=user.id, name=reg_data.fullName, role=role_name
    )
    db.add(profile)
    ...
    if reg_data.account_type == "owner":
        business_info = database.BusinessInfo(
            user_id=user.id, business_name=f"{reg_data.fullName}'s Car Wash"
        )
...
def update_location(
    db: Session, location_id: int, location: schemas.LocationCreate, user_id: int
):
    db_location = (
        db.query(database.Location)
        .filter(
            database.Location.id == location_id, database.Location.user_id == user_id
        )
        .first()
    )
    if db_location:
...
def get_products_services(
    db: Session, user_id: int, business_user_ids: TypingList[int] = None
):
    query = db.query(database.ProductService).filter(
...
def create_product_service(
    db: Session, product: schemas.ProductServiceCreate, user_id: int
):
...
def update_product_service(
    db: Session, product_id: int, product: schemas.ProductServiceCreate, user_id: int
):
    db_product = (
        db.query(database.ProductService)
        .filter(
...
def delete_invoice(db: Session, invoice_id: int, user_id: int):
    db_invoice = (
        db.query(database.Invoice)
        .filter(database.Invoice.id == invoice_id, database.Invoice.user_id == user_id)
        .first()
    )
    if db_invoice:
        db_invoice.status = "D"
...
def get_dashboard_stats(db: Session, business_user_ids: TypingList[int] = None):
    ...
    total_revenue = inv_query.with_entities(database.Invoice.total_amount).all()
...
def get_all_themes(db: Session, user_id: int, for_client: bool = None):
    query = db.query(database.CustomTheme).filter(
        database.CustomTheme.user_id == user_id
    )
...
def activate_theme(db: Session, user_id: int, theme_id: int):
    theme = (
        db.query(database.CustomTheme)
        .filter(database.CustomTheme.id == theme_id)
        .first()
    )
    if theme:
        # Deactivate only themes of the same type (staff or client)
...
def get_business_info(db: Session, user_id: int, owner_id: int = None):
    ...
    return (
        db.query(database.BusinessInfo)
        .filter(database.BusinessInfo.user_id == lookup_id)
        .first()
    )
...
def get_invoice_customization(db: Session, user_id: int):
    return (
        db.query(database.InvoiceCustomization)
        .filter(database.InvoiceCustomization.user_id == user_id)
        .first()
    )


def save_invoice_customization(
    db: Session, user_id: int, custom: schemas.InvoiceCustomizationCreate
):
...
def get_user_profile(db: Session, user_id: int):
    profile = (
        db.query(database.UserProfile)
        .filter(database.UserProfile.user_id == user_id)
        .first()
    )
    if not profile:
...
def get_user_preferences(db: Session, user_id: int):
    pref = (
        db.query(database.UserPreference)
        .filter(database.UserPreference.user_id == user_id)
        .first()
    )
...
def update_user_preferences(
    db: Session, user_id: int, pref_update: schemas.UserPreferenceUpdate
):
...
def get_cart_items(db: Session, client_id: int):
    ...
    return (
        ...
        .options(joinedload(database.CartItem.product_service))
        .filter(database.CartItem.client_id == client_id)
        .all()
    )
...
def add_to_cart(db: Session, client_id: int, product_service_id: int, quantity: int):
    product = (
        db.query(database.ProductService)
        .filter(database.ProductService.id == product_service_id)
        .first()
    )
    if not product:
        return None
...
def update_cart_item(db: Session, item_id: int, client_id: int, quantity: int):
    cart_item = (
        db.query(database.CartItem)
        .filter(
            database.CartItem.id == item_id, database.CartItem.client_id == client_id
        )
        .first()
    )
    if cart_item:
...
def get_order(db: Session, order_id: int):
    ...
    return (
        ...
        .options(
            joinedload(database.Order.items).joinedload(
                database.OrderItem.product_service
            )
        )
        .filter(database.Order.id == order_id)
        .first()
    )
...
def create_reservation(
    db: Session, client_id: int, service_id: int, location_id: int, vehicle_plate: str
):
    service = (
        db.query(database.ProductService)
        .filter(database.ProductService.id == service_id)
        .first()
    )
    ...
    max_position = (
        ...
        .with_entities(database.Reservation.queue_position)
        .order_by(database.Reservation.queue_position.desc())
        .first()
    )
...
def get_reservation(db: Session, reservation_id: int):
    ...
    return (
        ...
        .filter(database.Reservation.id == reservation_id)
        .first()
    )
...
def update_reservation_status(db: Session, reservation_id: int, status: str):
    ...
    if status in ["completed", "cancelled"] and old_status in [
        "pending",
        "accepted",
        "in_progress",
    ]:
        ...
        if old_position:
            db.query(database.Reservation).filter(
                ...
                {
                    database.Reservation.queue_position: database.Reservation.queue_position
                    - 1
                }
            )
...
def get_queue(db: Session, location_id: int):
    return (
        ...
        .order_by(database.Reservation.queue_position)
        .all()
    )
...
def create_payment_method(db: Session, name: str, icon: str, is_active: bool):
    pm = database.PaymentMethod(
        name=name, icon=icon, is_active=is_active, created_at=datetime.now()
    )
    db.add(pm)
    db.commit()
    db.refresh(pm)
...
def update_payment_method(
    db: Session, pm_id: int, name: str, icon: str, is_active: bool
):
    pm = (
        db.query(database.PaymentMethod)
        .filter(database.PaymentMethod.id == pm_id)
        .first()
    )
    if pm:
        pm.name = name
...
def validate_password_reset_token(db: Session, token: str):
    ...
    user = (
        db.query(database.User).filter(database.User.id == reset_token.user_id).first()
    )
    return user
...
def invalidate_reset_token(db: Session, token: str):
    reset_token = (
        db.query(database.PasswordResetToken)
        .filter(database.PasswordResetToken.token == token)
        .first()
    )
    if reset_token:
...
def get_or_create_firebase_user(db: Session, email: str, name: str = None):
    ...
    if not user:
        ...
        client_role = (
            db.query(database.Role).filter(database.Role.name == "client").first()
        )
        if client_role:
        ...
        profile = database.UserProfile(
            user_id=user.id, name=profile_name, role="client"
        )
        db.add(profile)
        db.commit()
        db.refresh(user)
...
# Subscription CRUD
def get_business_subscription(db: Session, business_number: str):
    """Get subscription for a business."""
    return (
        db.query(database.Subscription)
        .filter(database.Subscription.business_number == business_number)
        .first()
    )


def activate_trial(db: Session, business_number: str):
    """Create a trial subscription for a business."""
    subscription = database.Subscription(
        business_number=business_number,
        plan_type="pro",  # Trial gives Pro-level access
        status="trial",
        is_trial=True,
...
def update_subscription_from_webhook(
    ...
    """Update subscription state from Stripe webhook."""
    subscription = get_business_subscription(db, business_number)
    if subscription:
...
def mark_onboarding_completed(db: Session, user_id: int):
    """Set onboarding_completed=True on user."""
    user = db.query(database.User).filter(database.User.id == user_id).first()
    if user:
...
def get_onboarding_status(db: Session, user_id: int) -> bool:
    """Return onboarding_completed flag for a user."""
    user = db.query(database.User).filter(database.User.id == user_id).first()
    if user:
